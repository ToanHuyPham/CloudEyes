...
def test_sample_matches_schema() -> None:
    validator = Draft202012Validator(
        _schema("schemas/sample/sample.schema.json"), format_checker=FormatChecker()
    )
...
def test_provider_report_matches_schema() -> None:
    ...
    samples = tuple(
        make_sample(f"sample-{index}", created_at=start + timedelta(days=index))
    ...
    validator = Draft202012Validator(
        _schema("schemas/report/provider-report.schema.json"), format_checker=FormatChecker()
    )
