...
from cloudeyes_core.serialization import dump, dumps, load_sample
...
def run_analyze(
    *,
    ...
    """Load local sample JSON files and emit deterministic provider analytics."""

    try:
        paths = _sample_paths(inputs)
        samples = tuple(load_sample(path) for path in paths)
        bundle = analyze_provider_analytics(samples, expected_metrics=expected_metrics)
