"""End-to-end execution of CloudEyes Database Profile v1."""
...
from .benchmarks import DatabaseBenchmarkResult, benchmark_database_profile
...
def _measurement(
    *,
    ...
    started_at = clock()
    try:
        result: DatabaseBenchmarkResult = benchmark_database_profile(
            config=config,
            work_dir=work_dir,
        ...
        if raw_output_dir is not None:
            ...
            path = write_raw_output(
                ...
                stem=f"{sample_id}-database",
            )
            raw_output_path = path.as_posix()
        if cancellation_token is not None:
            cancellation_token.checkpoint()
        measurement = Measurement(
            measurement_id=f"{sample_id}-database",
            tool="python-sqlite-profile",
        ...
        measurement = Measurement(
            ...
            error=f"{type(error).__name__}: database benchmark failed",
        )
        return measurement, ()


def run_database_profile(
    *,
    config: DatabaseProfileConfig | None = None,
    ...
    """Discover the host, run the SQLite workload, and build one Core sample."""

    if cancellation_token is not None:
        cancellation_token.checkpoint()

    selected_config = config or DatabaseProfileConfig()
    discovered = discovery or discover_all()
    resolved_sample_id = sample_id or f"database-{uuid.uuid4().hex}"
    ...
    quality = evaluate_sample_quality(
        ...
        invalid_error="database_measurement_failed",
        policy=ReliabilityPolicy(max_measurement_seconds=300.0),
    )
