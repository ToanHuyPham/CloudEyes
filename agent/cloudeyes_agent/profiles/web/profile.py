"""End-to-end execution of the CloudEyes Web Profile v1."""
...
from .benchmarks import WebBenchmarkResult, benchmark_web_profile
from .config import WebProfileConfig
...
def _measurement(
    *,
    ...
    started_at = clock()
    try:
        result: WebBenchmarkResult = benchmark_web_profile(
            config=config,
        ...
        if raw_output_dir is not None:
            ...
            path = write_raw_output(
                ...
                stem=f"{sample_id}-web",
            )
            raw_output_path = path.as_posix()
        if cancellation_token is not None:
            cancellation_token.checkpoint()
        measurement = Measurement(
            measurement_id=f"{sample_id}-web",
            tool="python-web-profile",
        ...
        measurement = Measurement(
            ...
            error=f"{type(error).__name__}: web benchmark failed",
        )
        return measurement, ()
...
def run_web_profile(
    *,
    config: WebProfileConfig | None = None,
    ...
    """Discover the host, benchmark one web endpoint, and build one Core sample."""

    if cancellation_token is not None:
        cancellation_token.checkpoint()

    selected_config = config or WebProfileConfig()
    discovered = discovery or discover_all()
    resolved_sample_id = sample_id or f"web-{uuid.uuid4().hex}"
    ...
    quality = evaluate_sample_quality(
        ...
        invalid_error="web_measurement_failed",
        policy=ReliabilityPolicy(max_measurement_seconds=180.0),
    )
