"""End-to-end execution of CloudEyes Compute Profile v1."""
...
def _measurement(
    *,
    ...
    started_at = clock()
    try:
        result: ComputeBenchmarkResult = benchmark_compute_profile(config=config)
        ...
        if raw_output_dir is not None:
            ...
            path = write_raw_output(
                ...
                stem=f"{sample_id}-compute",
            )
            raw_output_path = path.as_posix()
        measurement = Measurement(
            measurement_id=f"{sample_id}-compute",
            tool="python-compute-profile",
        ...
        measurement = Measurement(
            ...
            error=f"{type(error).__name__}: compute benchmark failed",
        )
        return measurement, ()
...
def run_compute_profile(
    *,
    ...
    """Discover the host, execute CPU workloads, and build one Core sample."""

    selected_config = config or ComputeProfileConfig()
    discovered = discovery or discover_all()
    resolved_sample_id = sample_id or f"compute-{uuid.uuid4().hex}"
    ...
    quality = evaluate_sample_quality(
        ...
        policy=ReliabilityPolicy(max_measurement_seconds=600.0),
    )
